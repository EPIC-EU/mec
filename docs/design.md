...
MEC is a Web application that runs on Google App Engine (GAE). Given above is an overview of the main components.
...
- **Test Driver**: MEC makes heavy use of automated regression testing. Test data is transmitted using JSON format.
...
The `Logic` component handles the business logic of MEC. In particular, it is responsible for:
...
The Common component contains common utilities used across MEC.
...
This component automates the testing of MEC.
...
This is how MEC testing maps to standard types of testing.
